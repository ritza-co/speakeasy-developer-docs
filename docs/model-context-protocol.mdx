...
<Callout title="Beta" type="info">
  ...
  maturity of MCP, which Anthropic is actively evolving. It does not
...
MCP is an open-source protocol developed by Anthropic for defining tools. It's being adopted by many AI agent platforms, like the AI assistant, [Claude](https://claude.ai/download), and the [Cursor](https://www.cursor.com/) code editor. A [full list of MCP clients](https://modelcontextprotocol.io/clients) is available on the MCP site.

TypeScript SDKs generated with Speakeasy can include an MCP server that allows them to be integrated with any supported MCP client.
...
### Enabling MCP server generation in your configuration
...
### Regenerating your SDK with an MCP server
...
It also adds instructions for installing the server in a new section beneath the `SDK Installation` section in your `README.md` file, and adds a `bin` section to your `package.json` file, which points to the executable entrypoint for the server.
...
- Explore the [MCP clients](https://modelcontextprotocol.io/clients) that can be integrated with your server
