...
Before connecting Gemini CLI to a Taskmaster MCP server, you first need to create one. Follow our guide to [creating a Taskmaster MCP server](/docs/gram/examples/creating-taskmaster-mcp-server).

## Connecting Gemini CLI to your Gram-hosted MCP server

You can connect your Gram-hosted MCP server with Gemini using the automated Gram CLI command, or configure it manually.

### Quick Setup with Gram CLI (Recommended)

The easiest way to add your Gram-hosted MCP server to Gemini CLI is using the `gram install` command:
```bash
gram install gemini-cli --toolset your-taskmaster-slug
```

This command automatically creates or updates your Gemini CLI configuration file with the correct MCP server settings.

#### Setting up authentication

You'll need to configure the `MCP_TASK_MASTER_API_KEY` environment variable for authentication. Set it in your shell environment before running the install command:
```bash
