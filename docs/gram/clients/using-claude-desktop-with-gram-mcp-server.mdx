...
Once your Taskmaster MCP server is ready, there are three ways you can connect it to Claude Desktop:

- Use the Gram CLI for automated installation.
...
## Connecting Claude Desktop using the Gram CLI (Recommended)

The easiest way to add your Gram-hosted MCP server to Claude Desktop is using the `gram install` command:
```bash
gram install claude-desktop --toolset your-taskmaster-slug
```

This command generates a `.mcpb` file in your machine Downloads folder that you can open to install the MCP server in Claude Desktop.

### Installing the MCP server

Once the `.mcpb` file is generated, double-click the file to install the MCP server in Claude Desktop. Claude Desktop will prompt you to confirm the installation and set the `MCP_TASKMASTER_API_KEY` environment variable value.

![Screenshot showing the Claude Desktop MCP server installation prompt](/assets/docs/gram/img/guides/claude-desktop/claude-desktop-mcp-install.png)

## Connecting Claude Desktop using the MCP configuration file
...
## Connecting Claude Desktop using Claude Connectors
...
If you need to use an authenticated server or an environment variable, you need to use the Gram CLI or MCP configuration file method described above.
...
All three methods connect Claude Desktop to your Gram-hosted MCP server, but they have different use cases:

- **Gram CLI:** The recommended method for most users. It provides automated installation and supports both public and authenticated servers.
- **MCP configuration file:** Use this method if you need more control over the configuration or prefer manual setup.
- **Claude Connectors:** Use this method for a simpler setup with public MCP servers only. It's user-friendly but doesn't support authenticated servers.
...
- If using the Gram CLI, ensure the `.dxt` file was generated successfully and try opening it again.
