...
[Gram](/product/gram) makes setting up MCP servers simple by hosting servers in the cloud, so you don't need to handle local configurations, manage infrastructure, or deal with authentication complexity. With Gram, you can turn any OpenAPI document into a fully functional MCP server in minutes.
...
- An MCP client (This guide covers Cursor, VS Code with Copilot, Windsurf, and Amp. For Claude Desktop, see the [dedicated Claude Desktop guide](/docs/gram/clients/using-claude-desktop-with-gram-mcp-server).)

## Creating an MCP server

Before connecting your IDE to a Taskmaster MCP server, you first need to create one. Follow our guide to [creating a Taskmaster MCP server](/docs/gram/examples/creating-taskmaster-mcp-server).
...
You can install your Gram-hosted MCP server in Cursor using the automated Gram CLI command, or configure it manually.

### Quick Setup with Gram CLI (Recommended)

The easiest way to add your Gram-hosted MCP server to Cursor is using the `gram install` command:
```bash
gram install cursor --toolset your-taskmaster-slug
```

This will open Cursor and prompt you to install the MCP server.

![Gram opening Cursor to install the MCP server](./assets/opening-cursor-to-install-mcp-server.png)

### Setting up authentication

You'll need to configure the `MCP_TASK_MASTER_API_KEY` environment variable for authentication. You can do this in two ways:

**Option 1**: Set it in your shell environment before running the install command:
```bash
export MCP_TASK_MASTER_API_KEY='your-api-key-value'
gram install cursor --toolset your-taskmaster-slug
```

**Option 2**: Configure it directly in Cursor's MCP configuration screen after installation.

![Screenshot showing the MCP configuration screen in Cursor](./assets/mcp-configuration-screen.png)

### Manual setup

Alternatively, you can configure Cursor manually using the built-in configuration interface.
...
Cursor's global MCP configuration file will open. Add your Taskmaster MCP server configuration from the [Creating a Taskmaster MCP server](/docs/gram/examples/creating-taskmaster-mcp-server) guide.
...
Paste in the URL of your Taskmaster MCP server from the [Creating a Taskmaster MCP server](/docs/gram/examples/creating-taskmaster-mcp-server) guide and enter the server name.
...
In the `mcp_config.json` file that opens, replace the contents with your Taskmaster MCP server configuration from the [Creating a Taskmaster MCP server](/docs/gram/examples/creating-taskmaster-mcp-server) guide:
...
Add the `amp.mcpServers` setting with your Taskmaster MCP server configuration from the [Creating a Taskmaster MCP server](/docs/gram/examples/creating-taskmaster-mcp-server) guide to the configuration file.
