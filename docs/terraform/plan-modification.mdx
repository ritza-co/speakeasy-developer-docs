...
- A plan modifier is a type that conforms to the `terraform-plugin-framework` expected interface. A unique plan modifier is bootstrapped in the appropriate subfolder for the Terraform type that it is applied to: `boolplanmodifiers`, `float64planmodifiers`, `int64planmodifiers`, `listplanmodifiers`, `mapplanmodifiers`, `numberplanmodifiers`, `objectplanmodifiers`, `setplanmodifiers`, or `stringplanmodifiers`. Speakeasy always creates and uses a file as `snake_case.go` for a given `x-speakeasy-plan-modifiers` value.
...
- It is possible to have an array of plan modifiers, for example, `x-speakeasy-plan-modifiers: [FirstModifier, SecondModifier]`.
