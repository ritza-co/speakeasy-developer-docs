...
A plan modifier is a type that conforms to the `terraform-plugin-framework` expected interface. A unique plan modifier is bootstrapped in the appropriate subfolder for the Terraform type that it is applied to, which is usually one of the following:
...
Speakeasy always creates and uses a file as `snake_case.go` for a given `x-speakeasy-plan-modifiers` value.
