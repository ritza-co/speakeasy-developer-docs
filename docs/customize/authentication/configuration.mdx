...
Operation-specific security configuration overrides the default authentication settings for an endpoint. This is commonly used for operations that do not require authentication or that are part of an authentication flow, such as for obtaining a short-lived access token.
...
You can then provide global parameters via environment variables in the format `{PREFIX}_{GLOBALNAME}`. The relevant documentation will automatically be included in the README.
