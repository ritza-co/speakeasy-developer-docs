...
Because only one line (representing a single JSON object or value) needs to be in memory for parsing at any given time, JSONL has a low memory footprint compared to the JSON footprint for parsing a large standard array. This makes it feasible to work with terabyte-scale datasets on machines with limited memory.
...
2. **Iterate and Serialize:** Process your data record by record (for example, from a database query, an LLM token stream, or any other iterable source). Convert each record into a JSON string.
...
1. Save it as a Python file (for example, `app.py`).
2. Install Flask using `pip install Flask`.
3. Run the app using `python app.py`.
4. In another terminal, use `curl` to access the stream: 
...
5. Watch as each JSON object is printed on a new line. The objects appear one by one, just as intended with JSONL.
...
- The `-N` or `--no-buffer` for `curl` disables buffering of the output stream, allowing us to see streaming behavior.
- The `-s` silences progress output from `curl`.
- The `-o /dev/null` discards the actual response body so it doesn't clutter the terminal (we're interested in timings here).
- The `-w "..."` formats the output from `curl` after the transfer:
...
We've seen the numbers, but we know that the perception of something as "fast" or "slow" is subjective. So let's look at the qualitative differences in how data arrives.
...
- For JSONL streaming (see data appear line by line):
  ...
  You'll see data appear line by line. Each JSON line prints about 0.5 seconds after the previous line.
...
- For traditional JSON:
  ...
  You'll see there is a pause of about 3 seconds, then the entire JSON array is printed at once.
