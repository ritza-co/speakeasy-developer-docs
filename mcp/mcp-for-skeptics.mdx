...
But the underlying API can be just about anything: a database, a web service, a file system, or even a custom tool. MCP servers wrap these APIs in a way that makes them predictable from the client's perspective. The point is that client code is not tied to a specific API implementation.
...
<CardGrid
  ...
  cards={[
  {
    ...
    description: "REST, OpenAPI, and agents.json are stateless and unidirectional. The client would need to manage state and rely on polling or re-querying to maintain context.",
    variant: "basic",
    icon: "x",
  },
  {
    title: "MCP",
    description: "MCP is stateful, dynamic, and bidirectional. It allows LLMs to interact with tools and services in a way that maintains context and state across multiple interactions.",
    variant: "basic",
    icon: "check",
  },
]} />
...
<CardGrid
  ...
  cards={[
  {
    ...
    description: "All integration logic lives in the LLM client. Each new service requires client-side implementation.",
    variant: "basic",
    icon: "x",
  },
  {
    title: "MCP",
    description: "The client speaks one protocol to multiple specialized servers. New integrations require no client changes.",
    variant: "basic",
    icon: "check",
  },
]} />
...
<CardGrid
  ...
  {
    title: "Function calls",
    description: "Each LLM provider has its own function call interface, leading to vendor lock-in and limited portability.",
    variant: "basic",
    icon: "x",
  },
  {
    title: "MCP",
    description: "The MCP protocol is vendor-agnostic and portable. Any LLM client that supports MCP can connect to any MCP server.",
    variant: "basic",
    icon: "check",
  },
]} />
...
With function calling, the client would need to handle each LLM provider's function call interface, for example:
...
Note the subtle differences in how each provider defines the function. The complexity of function calling grows with each new provider, and the client becomes tightly coupled to specific implementations. This makes it hard to switch providers or reuse code across different LLMs.
...
With MCP, the client can query the server for available tools at any time, and the server can dynamically add or remove tools without requiring client changes.
...
const sendChatTool = mcpServer.registerTool("sendChat", {
...
whatsAppService.on("authenticated", () => {
  ...
  // The SDK automatically notifies the client of the tool changes by sending a
  // `notifications/tools/list_changed` message.

  // The client can then call `tools/list` to get the updated list of tools
...
Implementing dynamic tool discovery with MCP is straightforward. The server can add or remove tools at runtime, and the client can discover these changes without needing to reconnect or reinitialize.

Doing the same with function calls would require a custom implementation that tracks available functions and updates the client whenever they change. This is possible, but it adds significant complexity to both the client and server codebases.
...
With MCP, the server can maintain state across multiple tool calls, allowing for an asyncchronous, multi-step workflow that builds on previous interactions. The LLM can focus on reasoning and analysis, while the MCP server handles the underlying state management.
...
whatsAppService.on("analysisComplete", (analysisId, result) => {
  ...
  // The client can then react to this notification, for example by
...
We included an example of this in the previous section, where the WhatsApp MCP server notifies the client when a chat analysis is complete. The client can then react to this notification, for example by including a message in the next LLM call.
...
The tooling is ready to use. The TypeScript MCP SDK provides a dead-simple `ProxyOAuthServerProvider` class for servers that need to implement OAuth 2.1 authorization. On the client side, the SDK provides a `OAuthClientProvider` interface that handles the OAuth 2.1 flow for client developers.
