...
The **2025-06-18** release represents a significant refinement of the Model Context Protocol, introducing structured tool outputs, enhanced OAuth security, and server-initiated user interactions.
...
# Using official Python SDK - mcp package
from mcp.server import Server
from mcp.types import CallToolResult, ElicitationRequest

server = Server("booking-system", capabilities={"tools": {}, "elicitation": {}})

@server.call_tool()
async def handle_call_tool(name: str, arguments: dict, context) -> CallToolResult:
    if name == "complex_query":
        ...
        if missing_context(query):
            ...
            result = await context.elicit(elicitation_request)
            
            if result.action == "accept" and result.data:
                return CallToolResult(
                    content=[{"type": "text", "text": process_with_context(query, result.data["format"], result.data.get("include_metadata", False))}]
                )
            else:
                return CallToolResult(
                    content=[{"type": "text", "text": "User declined to provide additional context or elicitation was cancelled"}]
                )
        
        return CallToolResult(
            content=[{"type": "text", "text": process_query(query)}]
        )
...
const server = new Server(
  { name: "booking-system", version: "1.0.0" },
...
server.setRequestHandler(ListToolsRequestSchema, async () => {
  return {
    tools: [
      {
        name: "complex_query",
        description: "Process a query, requesting additional details if needed",
        inputSchema: {
          type: "object",
          properties: {
...
function processWithContext(query: string, format: string, includeMetadata?: boolean): string {
  // Your implementation here
  return `Processed query '${query}' in ${format} format${includeMetadata ? ' with metadata' : ''}`;
}
