...
This cognitive overhead slows down an agent's ability to complete a task by making it more difficult to reason about the best tool to use. But how to solve this? First, let's understand how agents use interfaces.
...
## The solution to tool proliferation

There are essentially two ways to tackle tool proliferation: client-side curation and server-side design.

### Client-side curation: What users can do now
...
### Server-side design: What MCP builders should do

The better approach is building focused MCP servers from the ground up. Instead of exposing every possible Playwright capability, server builders should create purpose-built servers for specific use cases.
...
## How to build more focused MCP servers

Here are some practical ways to curate browser automation tools into effective toolsets.
...
This is a good starting point for figuring out which tools are needed for a specific task and deciding which tools are essential and which are not. This way, 
...
### Provide rich context with MCP extensions

Adding on to this, you can use tool descriptions to provide more context for the agent to use when deciding which tool to use. For example, you can use Speakeasy's MCP extensions to help organize tools and provide context for the agent to use when deciding which tool to use.
...
## Design principles for MCP server builders

Instead of expecting users to curate tools client-side, MCP server builders should design focused servers from the start. The key is applying the [80/20 rule](https://en.wikipedia.org/wiki/Pareto_principle) by identifying the 20% of functionality that handles 80% of user workflows and make these your core tools.
