...
Now that you have everything set up, you can deploy the server to Cloudflare Workers. Run the following command:
...
Click **Approve** to authorize the MCP Inspector to access your Sentry account. This will redirect you back to the MCP Inspector, and you should see a message indicating that the connection was successful.
...
To allow the Lambda function to authenticate with the Polar API, you need to generate an organization access token from the Polar dashboard. The steps below describe this process, referencing the user interface elements shown in the screenshot that follows.
...
7. Polar will display the generated access token only once upon creation. Copy this token and store it. You will need this value for the `POLAR_ACCESS_TOKEN` environment variable when you configure your Lambda function in a later step.
...
7. Configure the application:
   ...
   - **Application domain**: Choose a subdomain where your MCP server will be available (e.g., `polar-mcp.yourdomain.com`)
...
9. Finish by clicking **Save** to create the application.
...
Open the Cloudflare Warp app, and navigate to the **Preferences** tab.
...
2. Enter your MCP server URL: `https://polar-mcp.yourdomain.com/mcp`
3. Click **Connect**.
