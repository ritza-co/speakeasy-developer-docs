...
- You can package your MCP server as an installable [MCPB](https://github.com/anthropics/mcpb) (formerly DXT) bundle.
- You can deploy your MCP server remotely, and then make it public or private but this method may require financial and developer resources.
...
In this guide, you'll learn how to distribute your MCP server by exploring each option, when to use each option, and tools that make the process easier.

## Prerequisites

You need to know a bit about [MCP transports](/mcp/building-servers/protocol-reference/transports) as they determine how the MCP server receives inputs and sends output.
...
Add this configuration to Claude Desktop **Settings -> Developer -> Edit Config** and replace `<USERNAME>` and `<PATH-TO-PROJECT>` with your actual username and the absolute path to the project.
...
You will have a output similar to the following.
...
Once done, make sure the `package.json` file is ready for packaging. Add a name, description, author name, author email address, keywords, entry point (main) and the CLI command `bin` to run the project.
...
Once it's published, you can show your users how to install the MCP server in Claude Desktop with a code snippet like this one they will enter in the `claude_desktop_config.json` file.
...
Add the configuration above to your `claude_desktop_config.json` file. Reload Claude Desktop and you should see the MCP server in the chat.
...
To make release and distribution easier, you should consider CI/CD tools to automate the process. You can find a GitHub Action for the automation [here](https://github.com/speakeasy-api/examples/tree/main/mcp-echo-input-server/.github/workflows/npm-release.yml).

## Distributing your MCP server as an MCPB bundle
...
### How to package your MCP server as an MCPB bundle

As of September 2025, Anthropic has renamed the DXT project to MCPB. We'll use the latest MCPB extension and commands from the project. You can find the instructions for migrating from DXT at the beginning of the [README](https://github.com/anthropics/mcpb/blob/main/README.md) in the MCPB repository.
...
After running the command, you should see a file called `mcp-echo-input-server.mcpb` in the project directory. Let's open the file with Claude and install the MCP server.
...
Once the name is entered, save the changes and toggle the switch to enable the extension.

![Enable extension](./assets/toggle-enabled.png)

Open a new conversation in Claude. Make sure the MCP server is activated.
...
We have a working `.mcpb` file that you can upload to artifact repositories so users can download and install your MCP server. If you're using CI/CD pipelines, we have a GitHub Action [here](https://github.com/speakeasy-api/examples/tree/main/mcp-echo-input-server/.github/workflows/mcpb-release.yml) that builds your `.mcpb` file and publishes it to the GitHub registry automatically on new releases.
...
- Simple installation in Claude Desktop where remote MCP servers can be added as Connectors.

  ![Connectors](./assets/claude-connectors.png)

  If your users are technical, then you can provide them documentation so they can enter OAuth configurations if the server requires authentication. And if they are not technical, once the server is added, there is a connect button that they can click, that will redirect to an OAuth authorization page, you will have coded to give access to your API or services.
...
MCP server distribution strategies depend on target users and available resources. You can follow the recommendations below according to your use case:
