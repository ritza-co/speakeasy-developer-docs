...
MCP servers sit between LLM agents and external services, such as databases, APIs, or internal tools. When something breaks, you need to know where: Is your MCP server failing? Is the LLM making bad tool calls? Is the latency from your backend services or LLM processing?
...
- **Tool name and timestamp** to identify which tool was called and when. This allows you to correlate tool calls with user complaints or backend issues that coincide.
...
- **Success or failure status** to track failure rates per tool. This identifies which tools are unreliable and need optimization or better error handling.

- **Client or session identifier** to link tool calls to specific users or sessions. When a user reports an issue, you can trace their entire interaction history across multiple tool calls.
...
- **Tool call sequences** to identify which tools are commonly used in workflows. When agents consistently chain tool A with tool B, these patterns reveal optimization opportunities – see our [optimization guide](/mcp/productizing-mcp/optimizing-mcp-servers).
...
For security-focused monitoring, such as authentication failures and unauthorized access attempts, refer to our [guide on securing MCP servers]().
...
# Server-side logging (you see this in runtime)
...
async def search_data(query: str, ctx: Context):
    # Server-side log (appears in your logs)
    logger.info(f"Tool called: search_data", extra={"query": query})
    
    try:
        results = perform_search(query)
        logger.info(f"Search completed", extra={"result_count": len(results)})
    ...
    return await Sentry.startNewTrace(async () => {
      return await Sentry.startSpan(
        { 
          ...
          attributes: {
            "tool.name": name,
            "tool.params": JSON.stringify(args)
          }
        },
        async (span) => {
          try {
...
For complete implementation, including error handling, user context binding, and Cloudflare Workers setup, see [Sentry's MCP monitoring guide](https://sentry.io/blog/monitoring-mcp-server-production/).
...
TELEMETRY_ENDPOINT = os.getenv("MCP_TELEMETRY_ENDPOINT", "https://your-telemetry-endpoint.com")
...
Users need to know what data you collect and how to control it. You can document such information in README file. Telemetry should also be disabled by default. 

```bash
MCP_TELEMETRY_ENABLED=false
```

You should allow the user of your MCP server to set the value to True or False when configuring the MCP server. For a complete OpenTelemetry implementation with multiple language examples, see [SigNoz's guide on MCP observability](https://signoz.io/blog/mcp-observability-with-otel/).
