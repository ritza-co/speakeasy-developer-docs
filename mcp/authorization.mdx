...
Instead of building your own authorization system from scratch, you can integrate with existing identity providers like Auth0, Okta, GitHub, or even your own API's authentication layer. This lets you leverage established identity infrastructure and avoid duplicating user management.
...
#### Real-World Example: Sentry MCP with OAuth
...
6. **Provider Consent**: You'll see Sentry's consent screen showing what permissions the MCP server is requesting
...
8. **Using Tools**: When you call tools like `list_organizations`:
...
This setup creates a secure proxy pattern - the MCP server mediates between clients and the identity provider. There are actually two OAuth flows happening:

1. Between your client and the MCP server
2. Between the MCP server and Sentry
