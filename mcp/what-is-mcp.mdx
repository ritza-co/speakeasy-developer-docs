...
![A diagram shows multiple AI agents (Claude, VS Code, and Cursor), which each require separate custom integrations to connect to different tools (GitHub, PostgreSQL, and the user's file system) creating a complex web of M × N connections between agents and services.](./assets/what-is-mcp/before-mcp.svg)

In the diagram above, we see how each application (Claude, VS Code, Cursor) requires custom integrations to access various tools (GitHub, PostgreSQL, the user's file system). This creates a complex web of M × N connections that is difficult to maintain and scale.
...
![A diagram shows a simplified architecture with multiple AI agents (Claude, VS Code, and Cursor) connected through a single MCP layer to multiple tools (GitHub, PostgreSQL, the user's file system, and services). This demonstrates how MCP reduces complexity from M × N custom integrations to M + N standardized connections.](./assets/what-is-mcp/with-mcp.svg)

In the diagram above, we see how MCP simplifies the architecture. Each AI agent (Claude
, VS Code, Cursor) connects through a single MCP layer to access multiple tools (GitHub, PostgreSQL, the user's file system).

## How MCP works

MCP establishes a client-server architecture made up of:

- **MCP clients**: AI agents, applications, or development tools (like Claude, VS Code extensions, and Cursor)
...
When using MCP-compatible AI agents, like Claude, VS Code extensions, or Cursor, you get:
...
Imagine you want to use Claude to read a GitHub issue from your company's private repository and query your PostgreSQL database for related data.

**The manual process**

If this were a once-off task, you would typically need to:

1. Copy and paste the contents of the GitHub issue into the Claude desktop app.
