...
- An internal API with OpenAPI documentation (or a copy of the [License API](https://github.com/speakeasy-api/examples/license-api))
...
The Zendesk and Slack integrations already save time. Instead of customer support agents having to go through Zendesk and scroll through Slack channels, Claude gathers everything in seconds. But the customer support agents still have to switch to another system to handle the license issue.

The manual step is tedious. Customer support agents log in to the License dashboard, search through customer records, create new licenses, update existing ones, and handle other license operations when necessary. If your customer support team is handling dozens of tickets daily, this quickly adds up to hours of repetitive work.
...
Gram generates MCP servers from OpenAPI documents and handles the hosting. Upload your OpenAPI document, configure your environment variables, and the server is ready.
...
- Using the License MCP server you set up to create licenses for the customer having issues, and adding a comment to the Zendesk ticket.
