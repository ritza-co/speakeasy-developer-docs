...
This guide walks you through connecting your first MCP server to an AI agent in under 10 minutes. We'll use Claude as our AI client to add a file to a new project directory on your computer.
...
If you prefer to install the MCP server manually (and learn how to install other MCP servers), open Claude and follow these steps:
...
This opens a Finder or File Explorer window showing the configuration file for Claude. Open the `claude_desktop_config.json` file in a text editor.
...
Close Claude completely and then reopen it. The MCP server will now be available.
...
As shown in the video above, Claude executes the command using the MCP server, creating the folder and file on your desktop. The file contains the text `Hello, MCP!`.

## What just happened?

1. **Claude** sent your prompt to Claude's API along with a message indicating which tools it can use.
...
3. **Claude** sent the commands to the Desktop Commander MCP server.
...
In the end, you see a confirmation message in Claude notifying you that the folder and file have been created successfully:
