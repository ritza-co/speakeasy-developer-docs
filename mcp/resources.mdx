...
- `@list_resources()` exposes static resources or resource templates.
 - You return a list of `types.Resource` items for static URIs.
 - You return a list of `types.ResourceTemplate` items for dynamic, parameterized URIs.
...
If the resource were static – for example, if the client only tracks Apple stock – we'd return a list of `types.Resource` items instead:
