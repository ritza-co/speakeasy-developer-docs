...
- **[Python 3.11 or higher](https://www.python.org/downloads/)** – For running the Slack API server.
- **[Claude Desktop](https://claude.ai/download)** – For the MCP-powered workflow interface.
- **[ngrok](https://ngrok.com/)** – To expose your local API during development (or use any hosting provider).
- [HubSpot](https://hubspot.com/) private app.
- **[Gram account](https://app.getgram.ai/)** – To host your Slack MCP server.
- [Slack app](https://api.slack.com/apps) with user token containing these scopes:
...
Create a new Python project directory and set up a `requirements.txt` file with the dependencies needed for the Slack SDK and FastAPI:
...
[Gram](https://app.getgram.ai/) works with three key concepts: 
...
To find and edit the `claude_desktop_config.json` file, go to **Settings** in Claude Desktop, then navigate to **Developer** > **Edit Config**.

<Screenshot
  image={{
    src: "./assets/claude-desktop-config-settings.png",
    alt: "Claude Desktop configuration settings",
  }}
/>
...
## Create Slack app and user token

Before setting up the MCP server, create a Slack app to access workspace data for account management:

Navigate to [api.slack.com/apps](https://api.slack.com/apps) and click **Create New App**. Select **From scratch**, enter an app name like "Account Management Assistant" and select your workspace.

Navigate to **OAuth & Permissions** in the sidebar. Scroll to **User Token Scopes** and add these required scopes for account management functionality:
...
Click **Install to Workspace** at the top of the page. Authorize the app and copy the **User OAuth Token** that starts with `xoxp-`. This is the token you'll use as `${VALUE}` for the `MCP-SLACK-API-HTTP-BEARER` header in the Claude Desktop configuration above.
...
In HubSpot, navigate to **CRM -> Deals** and click on the related lead to see the results.
