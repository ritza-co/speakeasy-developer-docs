...
[17% of businesses](https://www.zippia.com/advice/crm-statistics/#:~:text=17%25%20of%20businesses%20say%20that%20manual%20data%20entry%20is%20their%20biggest%20challenge%20in%20using%20their%20current%20CRM.) say manual data entry is their biggest CRM challenge. Sales teams post call summaries in Slack, then manually copy that information into HubSpot, doubling their data entry work. Managers spot urgent opportunities in channels notifications but can't quickly prioritize them without logging into the CRM to update deal status. In the meantime, your competitor is closing deals because you are not prioritizing quickly in your workflow, and your sales team has to look for what is prioritized. Lead volume increases but conversion rates stay flat because urgent opportunities get buried in manual workflows.

What if emoji reactions could highlight priority messages? Your manager reacts to a Slack message with ‼️ and the sales rep asks Claude to show all high-priority messages and update the corresponding HubSpot deals.

In this article, you'll build an integration where Claude reads emoji-marked Slack messages and updates HubSpot records on command. Your account managers will be able to prioritize deals through emoji reactions on Slack messages.
...
- Adding notes and updating deals status manually.
...
Instead, you can ask Claude to find all urgent messages and update your CRM records. You ask Claude "show me all high-priority messages marked with ‼️ reaction in Slack," review the urgent calls, then ask Claude to "add notes to the HubSpot deal for those urgent calls." Claude uses MCP servers to read Slack reactions, find messages, locate HubSpot deals, and update records.
...
- GET` /messages/search` – Search for sales call notifications and follow-up messages across channels
- GET` /channels` – List accessible channels where sales notifications are posted
- GET` /users/{user_id}` – Get information about sales team members and managers
- GET `/channels/{channel_id}/messages` – Retrieve messages from specific sales channels
...
Click the **+ CREATE A TOOLSET** button on the Toolsets page. A modal will appear asking for a toolset name. Enter something like "Slack Tools" and click **Create**. You'll be redirected to a new page showing all available tools from your APIs. 
...
With the Slack MCP server ready and deployed, let's add the server in Claude Desktop. 

Click on the `MCP` tab and select the Slack MCP server to get access to deployment configurations. 
...
Replace `${VALUE}` with the the values of the Slack token and the API server URL, which will just be the ngrok URL. Replace `YOUR_GRAM_API_KEY` with your actual Gram API key.
...
[HubSpot](https://hubspot.com) has launched an MCP service to allow AI assistants like Claude to connect and interact with HubSpot data in real-time. Users can install the MCP server in Claude Desktop and give instructions.

### Create the HubSpot private application 

Before adding the HubSpot MCP server in Claude Desktop, you will create a HubSpot private application, assign scopes for the actions you want to allow Claude to execute, and retrieve the API key.
...
- Someone adds the ‼️ reaction to the message indicating high priority. 
- Using Claude Desktop, we ask for a summary of messages in the leads channel and the one with priority. 
- We then ask Claude to analyze the high-priority messages and add notes to the corresponding HubSpot deals indicating whether each lead looks promising or needs attention.
...
Claude found priority leads for us. Let's Claude to add a note to those leads and mark them as high priority.
...
In HubSpot, navigate to CRM -> Deals and click on the related to see the results.
...
We've built an account management system that turns manual CRM work into simple conversations with Claude. Sales managers can now use reactions in Slack to highlight important messages for Claude and Claude will read the messages and use HubSpot for updates without switching between tools or logging into different systems.

This shows how MCP makes business tools work together. Instead of building custom integrations or doing manual data entry, Claude can interact with both Slack and HubSpot.
