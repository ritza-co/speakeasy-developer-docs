...
Claude Desktop supports MCP installations using DXT files. The [`slack-mcp-server`](https://github.com/korotovsky/slack-mcp-server) releases provide DXT files you can install in Claude Desktop. Download the release file on [GitHub](https://github.com/korotovsky/slack-mcp-server/releases). The file will have a name like `slack-mcp-server-1.1.26.dxt` (the version number may vary).
...
- Disable the **Use Built-in Node.js for MCP** option in **Settings** > **Extensions** > **Advanced settings**
- Ensure auto-updates for extensions are disabled
...
- Verify both the `xoxc` and `xoxd` tokens are correctly entered in the configuration
- Check that the extension is enabled in **Settings** > **Extensions**
...
- Log into Slack in your browser to verify your session is still active
- Regenerate both tokens following the steps in "Getting your Slack `xoxc` and `xoxd` auth tokens"
