...
Rather than switching between your CRM, email platform, and other tools, you can use the Resend MCP Server to send emails, manage audiences, and create automated workflows directly from Claude Desktop using natural language commands.

This guide shows you how to connect the Resend MCP Server to Claude Desktop.

<div style="text-align: center;">
  <video src="./assets/resend-mcp-demo.mp4" autoplay muted playsinline></video>
</div>
...
![Screenshot of the Resend UI showing the navigation path: API Keys → Create API Key](./assets/create-api-key-button.png)
...
![Screenshot of the "Add API Key" dialog in the Resend UI](./assets/api-key-form.png)

Resend will display the API key. Copy and save it to use in the MCP server configuration in Claude Desktop.

![Screenshot of the "View API Key" modal in the Resend UI](./assets/copy-api-key.png)
...
Test the connection by asking Claude to list the current audiences in the Resend application:

```txt
Please use the Resend MCP Server to list the current audiences in my Resend application.
```

![Screenshot of the Claude Desktop UI showing the list of audiences in the Resend application](./assets/list-audiences.png)
...
```txt
Please send a short welcoming email to the user example@email.com from onboarding@resend.dev.
```

![Screenshot of the Claude Desktop UI showing an email was successfully sent](./assets/send-email-example.png)

## Conclusion

Now that you can send emails from Claude Desktop using the Resend MCP Server, try combining this functionality with other MCP servers. For example, you could turn [Slack threads](/mcp/using-mcp/slack-claude-quickstart) into newsletter content to email automatically using Resend.
