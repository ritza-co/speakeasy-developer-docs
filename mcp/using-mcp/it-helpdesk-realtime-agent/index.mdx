...
description: Build an IT helpdesk voice agent that checks ticket statuses in HubSpot and posts updates to Slack by curating a Tech Support MCP toolset with Gram and connecting it to a Realtime agent.
---
...
We'll use Gram to curate the perfect set of MCP tools from the HubSpot and Slack APIs and to host that toolset as a single MCP server. By connecting to this server, the voice agent gains real-world capabilities, not just conversational ones.
...
Before we can set up the IT helpdesk AI agent, we first need to create the Tech Support MCP server that it will connect to.
