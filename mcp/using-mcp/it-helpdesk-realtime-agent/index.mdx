...
- A Gram account
- A HubSpot Private App token (for accessing service tickets)
- A Slack authentication token (xoxp user token or xoxc/xoxd browser tokens)
- npm or npx installed for running the Slack MCP server

## Set up the MCP servers

Before we can set up the IT helpdesk AI agent, we need to configure the MCP servers that it will connect to.
...
Now, let's configure both the HubSpot MCP server and the Slack MCP server in the app.
...
- [The Slack MCP Server](https://github.com/korotovsky/slack-mcp-server)
