...
Replace `CREDENTIALS_PATH` with the path to your downloaded OAuth credentials. Replace `TOKEN_PATH` with the absolute path where the token will be stored. No need to pre-create the file, just ensure the directory is writable. The token can be in the same directory as the credentials file.RetryClaude does not have the ability to run the code it generates yet.
