...
To skip straight to [turning the API into an MCP server](#turning-the-api-into-an-mcp-server), find the complete Slack API server code in the [example repository](https://github.com/ritza-co/speakeasy-examples/slack-mcp-server-api) and follow the README to get a working application running.

### Implementation overview

Implementing this automation includes four steps:

- Building a Slack API wrapper that exposes message and thread data through HTTP endpoints.
- Deploying an MCP server on Gram using the API's OpenAPI document.
- Configuring Postiz MCP integration for social media scheduling.
- Creating an AI agent that connects to both Slack and Postiz MCP servers and handles the automated workflow.
...
In the same file define Pydantic models for data validation and automatic OpenAPI schema generation:
...
To see the tool definitions, navigate to **Toolsets** and click on the **Slack Tools**. There the tools listed in the **Slack API** source will be present: search messages, get threads, get users, list Slack channels, get channel messages.
...
Add the Slack bot token as `SLACK_API_HTTP_BEARER`. And add the ngrok URL as `SLACK_API_SERVER_URL`. These new variables will allow Gram to authenticate with Slack and route requests to the running API.

### Create a Gram API key

A Gram API key is needed to connect the agent to the MCP server. In the Gram dashboard, go to Settings and click **New API Key**.
...
While Postiz supports scheduling across multiple social media platforms, this guide uses only X (formerly Twitter). Postiz can connect to LinkedIn, Facebook, or other platforms depending on your needs.
...
When declaring the agent, make sure to set the `project` value to the name of the project where the toolset is hosted, otherwise the agent will not be able to find the tools.
