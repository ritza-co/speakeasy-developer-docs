...
Teams generate valuable insights in Slack discussions daily. Technical breakthroughs, market observations, and customer feedback make for excellent social media content. But extracting these insights from lengthy threads and manually creating posts is time-consuming.

What if you could simply add a 🚀 emoji to any valuable message and have a workflow automatically create and schedule social media posts? What would normally require custom webhook handling and API orchestration becomes a simple AI agent and MCP server setup.

In this article, you'll build a Slack MCP server using Gram, connect it with Postiz for social media scheduling, and create an AI agent that transforms your best team discussions into engaging social content with just an emoji reaction.

## Why use MCP? 

If you're already turning your best Slack threads into social posts, you're probably:

- Searching for valuable threads in your workspace.
...
If your team generates multiple high-value discussions weekly, this manual process **is** overwhelming and great content **is** probably buried in Slack instead of reaching your audience.

Of course, you could automate parts of the process by building custom integrations. For instance, you might write Python scripts that call the Slack API to retrieve messages, send them to OpenAI for analysis and content creation, then use function calling to structure responses and fire requests to [Postiz](https://docs.postiz.com/public-api) for scheduling. But the process would still be time-consuming: You'd still need to manually run scripts whenever you find a useful thread.

Instead, you can just turn your threads into social posts using just a reaction emoji. Here's how it works: The 🚀 emoji triggers a Slack event that hits your webhook endpoint, which runs an AI agent with access to MCP tools for both Slack and Postiz. The agent retrieves the thread content, analyzes it for key insights, composes engaging social media posts, and schedules them automatically.
...
With [Gram](https://getgram.ai/), you don't need to implement MCP protocol code. The platform automatically converts your OpenAPI document into a hosted MCP server so that you can focus on building your API.
...
* A Gram account – To host your Slack MCP server.
* Postiz account – To schedule social media posts (7-day free trial available).
* OpenAI API key – To power the AI agent that orchestrates the workflow.
* Ngrok – To expose your local API during development (or use any hosting provider).
* Slack app and user token with these scopes:
  ...
  * `groups:read` – To view basic information about user's private channel.
  * `reactions:read` – To detect emoji reactions.

### Quick start option

If you'd like to skip straight to [turning your API into an MCP server](#turning-your-api-into-an-mcp-server), find the complete Slack API server code in the [example repository](github.com/ritza-co/speakeasy-examples/slack-mcp-server-api) and follow the README to get a working application running.
...
## Building the Slack integration
...
Here's how we'll build the API:
...
Gram's `x-gram` extensions provide better context and instructions for the agent, letting you add LLM-optimized descriptions with context about when to use each tool and any prerequisites needed.
...
## Turning the API into an MCP Server
...
The `x-gram` extensions you added previously will define how Gram provides the tools to agents.
...
While Postiz supports scheduling across multiple social media platforms, this guide uses only X (formerly Twitter). You can connect Postiz to LinkedIn, Facebook, or other platforms depending on your needs.

First, ensure you have at least one social media account connected in Postiz. We'll add an X account, but you can add whichever platform you want to schedule posts to.
...
Copy the MCP URL and save it to the `.env` file in your FastAPI project as `POSTIZ_MCP_URL`. Your environment variables should now look like this:
...
We'll create the agent logic in a separate module, then add a webhook for emoji reactions.
...
### Adding the Slack events webhook
...
### Configuring the Slack Event URL
...
- In the Request URL field, enter your webhook endpoint:

```txt
https://your-ngrok-url/events
```

- Slack will send a verification challenge and the FastAPI `/events` endpoint should automatically handle the challenge. If the challenge is successful , you will see a green `Verified` text.
