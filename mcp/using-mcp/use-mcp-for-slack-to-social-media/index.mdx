...
What if adding the 🚀 emoji to valuable messages triggered a workflow that automatically created and scheduled social media posts? What once required custom webhook handling and API orchestration has become a simple AI agent and MCP server setup.

This article demonstrates how to build a Slack MCP server using [Gram](https://getgram.ai), connect it with [Postiz](https://docs.postiz.com/public-api) for social media scheduling, and create an AI agent that transforms team discussions into engaging social content with just an emoji reaction.
...
When teams have multiple high-value discussions weekly, this manual process becomes overwhelming, and great content is probably buried in Slack.

You can automate parts of the process with custom integrations. For instance, you could write Python scripts that call the Slack API to retrieve messages, then send them to OpenAI for analysis and content creation, and use function calling to structure responses and fire requests to [Postiz](https://docs.postiz.com/public-api) for scheduling. But manually running scripts whenever you found a useful thread would still be time-consuming.

Turning threads into social media posts with just a reaction is much quicker. Here's how it works:

- A team member reacts to a Slack thread with the 🚀 emoji.
- The 🚀 reaction triggers a Slack event that hits a webhook endpoint, which runs an AI agent with access to MCP tools for both Slack and Postiz.
- The agent retrieves the thread content, analyzes it for key insights, composes engaging social media posts, and schedules them automatically.

### The MCP advantage

MCP standardizes how LLMs access different tools. Instead of writing custom integrations for Slack, Postiz, and any other tools that need to be added in the future, there is one consistent interface that agents can use intelligently across any platform. Since MCP is an open protocol, it works with any LLM model or agent framework.

With [Gram](https://getgram.ai/), there's no need to implement MCP code. Gram automatically converts OpenAPI documents into hosted MCP servers, allowing developers to focus on building APIs.
...
To build this automated Slack-to-social media system, you need:

- [Python](https://www.python.org/) 3.11 (or a later version)
- A [Gram](https://getgram.ai/) account for hosting the Slack MCP server
- A [Postiz](https://platform.postiz.com/auth) account for scheduling social media posts (you can use the 7-day free trial)
- An [OpenAI API](https://openai.com/api/) key to power the AI agent that orchestrates the workflow
- [ngrok](https://ngrok.com/) (or another hosting provider) to expose a local API during development
- A [Slack](https://slack.com/) account with the appropriate permissions for adding apps to a workspace

### Quickstart option

To skip straight to [turning the API into an MCP server](#turn-the-api-into-an-mcp-server), download the complete Slack API server code from the [example repository](https://github.com/ritza-co/speakeasy-examples/slack-mcp-server-api), and follow the instructions in the README to get a working application running.

### Implementation overview

We'll implement this automation in four steps:

- Build a Slack API wrapper that exposes message and thread data through HTTP endpoints.
- Deploy an MCP server on Gram using the API's OpenAPI document.
- Configure the Postiz MCP integration for scheduling social media posts.
- Create an AI agent that connects to both the Slack and Postiz MCP servers and handles the automated workflow.

## Create the Slack app and bot token

First, we'll create a new Slack app to access workspace data.

Navigate to [api.slack.com/apps](https://api.slack.com/apps) and click **Create New App**. Select **From scratch**, name your app, and select a workspace from the dropdown.
...
Navigate to **OAuth & Permissions** in the sidebar. Scroll to **Bot Token Scopes** and add the required scopes:
...
Click **Install to \<workspace-name>** at the top of the page. Authorize the app and copy the **Bot User OAuth Token** that starts with `xoxb-`.
...
Now, let's create an API that retrieves Slack messages and threads for the agent. This API will serve as the backend for the MCP server and give the agent access to Slack data through HTTP endpoints.

The API exposes these `GET` endpoints for the agent to use to analyze Slack threads:

- `/messages/search`: Search for messages across Slack channels with advanced filtering options.
- `/messages/{channel_id}/{message_ts}/thread`: Retrieve a complete message thread including parent message and all replies.
- `/users/{user_id}`: Get detailed information about a Slack user.
- `/channels`: List all accessible Slack channels.
- `/channels/{channel_id}/messages`: Retrieve messages from a specific channel.
...
- Enriching the OpenAPI document with `x-gram` extensions that help Gram generate better MCP tool descriptions for the agent
...
Create a `requirements.txt` file with all the dependencies needed for the Slack API integration, the FastAPI web server, and the OpenAI Agents SDK with the Gram AI SDK for MCP integration:
...
```bash
mkdir app
touch .env app/__init__.py app/main.py
```

### Add the FastAPI app and Pydantic models

To set up authentication and validation for the FastAPI application, you need:

- **Bearer token authentication:** To access Slack securely
- **Pydantic models:** To structure API responses
- **Detailed endpoint documentation:** To convert into MCP tools using Gram's MCP conversion

In the `main.py` file, create the FastAPI app with Bearer token authentication:
...
In the same file, define the Pydantic models for data validation and automatic OpenAPI schema generation:
...
The FastAPI application now has proper authentication, data validation, and models. We'll use this to generate a clean OpenAPI document for the app that Gram can convert into MCP tools.

### Add the endpoints

Each endpoint handles a specific aspect of Slack data retrieval, from searching messages to retrieving complete threads.

Add the `/messages/search` endpoint for searching Slack messages across channels and filtering by text, user, channel, date range, or result limits:
...
Add the `/messages/{channel_id}/{message_ts}/thread` endpoint for retrieving complete conversation threads. This endpoint is essential for the AI agent to be able to understand the full context of discussions.
...
The endpoint above uses the Slack message timestamp (like `1234567890.123456`) as the unique identifier to fetch the complete thread conversation and separate the parent message from its replies for easier agent processing.

Add the `/users/{user_id}` endpoint for retrieving detailed user information. This endpoint helps AI agents understand message authorship and context when analyzing threads.
...
Add the `/channels` endpoint for listing all the accessible channels in the workspace. This endpoint allows AI agents to discover available channels and convert channel names to IDs for targeted searches and message retrieval.
...
Add the `/channels/{channel_id}/messages` endpoint for retrieving messages from a specific channel with optional date filtering and pagination. This endpoint enables AI agents to analyze recent channel activity or messages from specific time periods.
...
The code above uses Slack's `conversations_history` API with optional timestamp filtering to retrieve messages from a specific channel. It handles common errors like missing channels or insufficient permissions, then converts the raw message data into the `SlackMessage` model.
...
Gram's `x-gram` extensions provide better context and instructions for the agent, creating LLM-optimized descriptions that include context about when to use each tool and whether it has any prerequisites.
...
Next, override the default OpenAPI definition from FastAPI and add the logic to run the FastAPI app.
...
This script imports the FastAPI app, calls the `app.openapi()` method, and returns the dict version of the OpenAPI document.
...
Copy the HTTPS forwarding URL from ngrok. Later, you'll use this URL to configure the MCP server in Gram.
...
- **Tool definitions** describe each API endpoint to LLMs.
- **Tool variations** allow you to customize descriptions.
...
Click the **Get Started** button to add an API source. Upload the generated `openapi.json` file and name the API source `Slack API`. It is important that you use this name exactly to follow along with this guide.
...
On the next page, a modal asks you to create a toolset. Name it `Slack Tools` and click **Continue**.

On the next page, accept the default MCP server slug and click **Continue**.

To see the tool definitions, navigate to **Toolsets** and click on the **Slack Tools**.

Here, you can see the tools that were listed in the **Slack API** source:
...
Navigate to the **Environments** page and click **+ New Environment**. Name the environment `slack`.

After creating the environment, you're redirected to the environment configuration page. Click **New Variable** to add the Slack bot token and ngrok URL as environment variables.
...
Name Slack bot token `SLACK_API_HTTP_BEARER` and name the ngrok URL `SLACK_API_SERVER_URL`.

These new variables allow Gram to authenticate with Slack and route requests to the running API.

### Create a Gram API key

Next, we need a Gram API key to connect the agent to the MCP server.

In the Gram dashboard, go to **Settings** and click **New API Key**.
...
Copy the API key into the `.env` file of the FastAPI project. The `.env` file should currently look like this:
...
This creates an MCP server for Slack data access that exposes the endpoints as standardized tools.

Next, we need to configure Postiz for social media scheduling and build the agent that orchestrates the entire workflow.

## Connect to Postiz for social media posts scheduling

While Postiz supports scheduling across multiple social media platforms, this guide uses only X (formerly Twitter). Postiz connects to LinkedIn, Facebook, and other platforms, according to your needs.

First, ensure at least one social media account is connected in Postiz by clicking **Add Channel** on the dashboard.
...
Copy the MCP URL into the `.env` file in your FastAPI project, saving it as `POSTIZ_MCP_URL`. The environment variables should now look like this:
...
In the `app` folder, create a new file called `social_agent.py`.

Add the following AI agent code, which connects to the Slack and Postiz MCP servers:
...
The code above sets the AI agent up with access to both MCP servers: Gram's tools for Slack access and Postiz's direct MCP server for scheduling. The agent receives the channel and message details, then handles the complete workflow autonomously.

When declaring the agent, be sure to set the `project` value to the name of the project where the toolset is hosted; otherwise, the agent won't be able to find the tools.

### Add the Slack events webhook

To integrate the agent with FastAPI, we need a webhook endpoint for the Slack [Events API](https://api.slack.com/apis/events-api) to call when the emoji reaction is added to a message.
...
The endpoint above handles two critical Slack events: URL verification (required for setting up the webhook) and emoji reactions.
...
Before testing the integration, we need to configure Slack to reach the event endpoint and send emoji reaction notifications.

#### Configure event subscriptions

Go to the Slack app settings at [api.slack.com/apps](https://api.slack.com/apps).

Select the app, then navigate to **Event Subscriptions** in the left sidebar.
Use the toggle in the top right corner to **Enable Events**.

#### Add the request URL

In the **Enable Events** section, paste the webhook endpoint in the **Request URL** field:
...
Add the following `reaction_added` event.
...
Navigate to the **OAuth & Permissions** to ensure the bot has the required OAuth user scopes:

- `channels:history` reads channel messages.
- `channels:read` accesses channel information.
- `groups:history` accesses private channel history.
- `groups:read` views basic information about a user's private channel.
- `reactions:read` detects emoji reactions.
- `users:read` gets user profile details.

If new scopes were added, reinstall the app in the workspace.

With Slack configured to send reaction events, our complete automated workflow is ready for testing.

## Test the integration

Now let's test the automated workflow.

Find a Slack thread with valuable content, add the bot to the channel by typing `/invite @your-bot-name`, then add a 🚀 reaction to the parent message of that thread.

When you add the 🚀 reaction, Slack immediately sends an event to the `/events` endpoint. Monitor the activity in the ngrok terminal to see incoming requests from the Slack servers.
...
The agent then makes requests to the Gram-hosted MCP server to retrieve the thread content, analyze it for social media potential, create platform-appropriate posts, and schedule them through the Postiz MCP server. The FastAPI server logs the detection of the 🚀 reaction and triggers the AI agent workflow.
...
Check your Postiz dashboard to see the scheduled posts. The agent should have created Twitter-optimized content based on the thread's key insights and scheduled them for publication.
...
We now have a fully automated system that converts Slack discussions into social media content with just an emoji reaction. The integration uses two MCP servers, a custom Slack MCP server hosted on Gram and Postiz's direct MCP integration, and is orchestrated by an OpenAI agent that handles the entire workflow intelligently.

## Final thoughts

By combining a custom Slack API with Gram's MCP hosting and Postiz's direct MCP integration, we've created a workflow that requires zero manual intervention to turn Slack discussion into social media content: Just add a 🚀 reaction to a Slack message and watch team insights become X posts.
...
You can further enhance your integration by:

- **Adding more social media platforms:** Connect additional accounts in Postiz, such as LinkedIn for professional content and Facebook for broader reach.
- **Improving content quality:** Refine the agent's prompt to include brand voice, specific hashtag strategies, or content formatting preferences.
