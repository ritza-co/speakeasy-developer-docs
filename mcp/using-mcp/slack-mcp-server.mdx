...
What if you could simply add a 🚀 emoji to any valuable message and have a workflow automatically create and schedule social media posts? With MCP servers, this automation becomes straightforward.
...
If you're turning your best Slack threads into social posts, you're probably doing it by:
...
You could automate parts of the process by building custom integrations. For instance, you might write Python scripts that call the Slack API to retrieve messages, send them to OpenAI for analysis and content creation, then use function calling to structure responses and fire requests to [Postiz](https://docs.postiz.com/public-api) for scheduling. But you'd still need to manually run scripts whenever you find a useful thread.

Or, you can just turn your threads into social posts using just a reaction emoji 🚀. Then, with an OpenAI agent, you will give it tools exposed via MCP servers for both Slack and Postiz.
...
[Gram](https://getgram.ai/) eliminates the need to implement MCP protocol code by automatically converting your OpenAPI document into a hosted MCP server. You focus on building your API while Gram handles the MCP server creation and hosting.
...
- [Gram access](https://app.getgram.ai/) – For hosting your Slack MCP server.
- [Postiz account](https://postiz.com/) – For social media scheduling (7-day free trial available).
...
To automate the process of turning Slack threads into social media posts, you will need to create an API that will retrieve Slack messages and threads for the agent. This API will serve as the backend for the MCP server, giving the agent standardized access to Slack data through HTTP endpoints.
...
The command above creates a `pyproject.toml` file where you'll define the dependencies. We need the Slack SDK for API integration, FastAPI for the web server, and the OpenAI Agents SDK with Gram AI SDK for the MCP integration:
...
Inside `main.py`, implement your FastAPI application and define the API endpoints. The `__init__.py` file can remain empty for now, as it's used to indicate that `app` is a Python package.
...
Now you'll implement the endpoints the MCP server needs to provide comprehensive Slack access to the agent. Each endpoint handles a specific aspect of Slack data retrieval, from searching messages to retrieving complete threads.
...
This endpoint retrieves detailed user information that helps the agent understand message authorship and context when analyzing threads:
...
We have added all endpoints needed for the Slack API. Next, you'll add custom OpenAPI enhancements that help Gram generate better MCP tools from your endpoints.

### Customizing the OpenAPI output

Now , you will enhance the OpenAPI document for MCP server generation. Standard OpenAPI descriptions are more designed for humans than LLMs, so you'll add [Gram's](https://docs.getgram.ai/concepts/openapi#using-the-x-gram-extension) `x-gram` extensions to provide better context and instructions for the agent.

OpenAPI descriptions that are too verbose cause token bloat, while vague descriptions lead to poor tool selection. Gram's `x-gram` extension lets you add LLM-optimized descriptions with context about when to use each tool and any prerequisites needed.
...
You need to export the OpenAPI document in formats that Gram can import to create an MCP server. This script generates both JSON and YAML versions of the OpenAPI document. At the root of the project, create a file called `generate_openapi.py` and add the following content.
...
In the script above, we are importing the FastAPI app, and calling the `app.openapi()` method, which will return the dict version of the OpenAPI document. Run the generation script:

```bash
uv run python generate_openapi.py
```

The command executed outputs:
...
Copy the HTTPS forwarding URL from ngrok – you'll use the URL when configuring the MCP server in Gram. With the API running and OpenAPI files generated, you're ready to create the MCP server.

## Turning Your API into an MCP Server

Now you'll use Gram to convert your OpenAPI document into a hosted MCP server that agents can connect to. Gram eliminates the need to implement MCP protocol code manually by generating servers from OpenAPI documents.

Gram works with three key concepts: tool definitions that describe each API endpoint to LLMs, tool variations that let you customize descriptions, and toolsets that group related tools together. The `x-gram` extensions you added earlier will automatically enhance how Gram presents the tools to agents.
...
![Creating a Gram API key](./assets/creating-gram-api-key.png)

Copy the API key in your `.env` file of your FastAPI project. The `.env` file should currently look like this:
...
We have created an MCP server for Slack data access that exposes the endpoints as standardized tools. Next, you will configure Postiz for social media scheduling, then build the agent that orchestrates the entire workflow.

## Connecting to Postiz for social media posts scheduling

Now you will set up Postiz, which handles the social media scheduling portion of the automated workflow. Postiz provides a direct MCP server integration, so unlike our Slack API, you don't need to build a wrapper: you can connect to their MCP server directly.
...
First, ensure you have at least one social media account connected in Postiz. Add your X account or whatever platform you want to schedule posts to.
...
With the Slack MCP server hosted on Gram and Postiz MCP server configured, you will build the AI agent that orchestrates the entire workflow. The agent will receive emoji reactions events from Slack, analyze threads, and schedule social media posts automatically.
...
In the `app` Python package, create a new file called `social_agent.py`. This file will contain the AI agent code that connects to the Slack and Postiz MCP servers:
...
The code above sets up an OpenAI agent with access to both MCP servers: Gram's tools for Slack access and Postiz's direct MCP server for scheduling. The agent receives the channel and message details, then handles the complete workflow autonomously.
...
2. Slack will send a verification challenge and the FastAPI `/events` endpoint should automatically handle the challenge. If the challenge is successful , you will see a green `Verified` text.
...
When you add the rocket emoji, Slack will immediately send an event to your `/events` endpoint. You can monitor the activity in your ngrok terminal, where you'll see incoming requests from Slack's servers. 
...
You can enhance the integration in several ways:
