...
Gram eliminates the need to implement MCP protocol code by automatically converting your OpenAPI specification into a hosted MCP server. You focus on building your API while Gram handles the MCP server creation and hosting.
...
You can find the complete Slack API server code at [github.com/ritza-co/speakeasy-examples/slack-mcp-server-api](https://github.com/ritza-co/speakeasy-examples/slack-mcp-server-api). Follow the README to get a working application and jump directly to the [Turning Your API into an MCP Server](#turn-your-api-into-an-mcp-server) section.
...
- Deploy an MCP server on Gram using our API's OpenAPI specification
...
To automate the process of turning Slack threads into social media posts, we need to create an API that will retrieve Slack messages and threads for our AI agent. This API will serve as the backend for our MCP server, giving the agent standardized access to Slack data through HTTP endpoints.

The API will expose five core endpoints that cover everything our agent needs to analyze Slack threads:
...
- Enriching the OpenAPI specification with `x-gram` extensions that help Gram generate better MCP tool descriptions for our AI agent.

When complete, we'll have a REST API that our MCP server will expose as standardized tools, allowing the AI agent to retrieve Slack threads and analyze them for social media content creation.
...
This creates a `pyproject.toml` file where we'll define our dependencies. We need the Slack SDK for API integration, FastAPI for our web server, and the OpenAI Agents SDK with Gram AI for our MCP integration:
...
Create an `app` python package directory with `main.py` and `__init__.py`  files that will contain our FastAPI endpoints. With the project structure ready, we can implement the API endpoints.

### Adding FastAPI App and Pydantic Models

Now we'll set up the FastAPI application with proper data models and authentication. We need three key components: Bearer token authentication for Slack API access, Pydantic models for data validation, and well-documented endpoints that Gram can convert into MCP tools.
...
Now we'll implement the endpoints our MCP server needs to provide comprehensive Slack access to the AI agent. Each endpoint handles a specific aspect of Slack data retrieval, from searching messages to analyzing complete threads.
...
The code above constructs a Slack search query by combining the text query with optional filters, sends the request through the Slack SDK client, and returns structured message data that our MCP server will expose as a tool.

**GET /messages/{channel_id}/{message_ts}/thread**

This endpoint retrieves complete conversation threads, which is essential for our AI agent to understand the full context of discussions:
...
This endpoint fetches user profile information from Slack's API and formats it into our structured SlackUser model, giving the agent access to names, email addresses, and other profile details for better thread analysis.
...
This code makes separate API calls to retrieve public and private channels, then transforms the raw Slack data into our structured SlackChannel models with standardized fields.
...
This code uses Slack's `conversations_history` API with optional timestamp filtering to retrieve messages from a specific channel, handling common errors like missing channels or insufficient permissions, then converts the raw message data into our SlackMessage models.
...
Now we need to enhance our API's OpenAPI documentation for optimal MCP server generation. Standard OpenAPI descriptions aren't designed for LLM tool selection, so we'll add Gram's `x-gram` extensions to provide better context and instructions for our AI agent.
...
Great! We have added the FastAPI Slack API. Last step, let's add a script that will generate openAPI files in Json or YAML format, we can use to import it Gram. 

### Generating OpenAPI files

We need to export our API specification in formats that Gram can import to create our MCP server. This script generates both JSON and YAML versions of our OpenAPI specification. At the root of the project, create a file called `generate_openapi.py`.
...
Copy the HTTPS forwarding URL from ngrok – we'll use this when configuring our MCP server in Gram. With our API running and OpenAPI files generated, we're ready to create the MCP server.

## Turning Your API into an MCP Server

Now we'll use Gram to convert our FastAPI specification into a hosted MCP server that AI agents can connect to. Gram eliminates the need to implement MCP protocol code manually by generating servers from OpenAPI documents.

Gram works with three key concepts: tool definitions that describe each API endpoint to LLMs, tool variations that let you customize descriptions, and toolsets that group related tools together. The `x-gram` extensions we added earlier will automatically enhance how Gram presents our tools to AI agents.
...
Then, click on the **+ ADD API** button to add an API source. Upload the `openapi.json` file we generated and give your API source a descriptive name like **Slack MCP API**.
...
You can see the tool definitions we created from the **Slack MCP API** API source: search messages, get threads, get users, list slack channels, get channel messages. Click **Enable All** to include all tools in your toolset.
...
![Adding environment variables](/Users/koladev/ritza/speakeasy-developer-docs/mcp/using-mcp/assets/adding-environment-variables.png)
...
We have created an MCP server for Slack data access that exposes our endpoints as standardized tools. Next, we need to configure Postiz for social media scheduling, then build the AI agent that orchestrates the entire workflow.

## Connecting to Postiz for Social Media Scheduling

Now we need to set up Postiz, which handles the social media scheduling portion of our automated workflow. Postiz provides a direct MCP server integration, so unlike our Slack API, we don't need to build a wrapper – we can connect to their MCP server directly.
...
With both MCP servers configured – Gram hosting our Slack tools and Postiz providing direct scheduling access – we can now build the AI agent that connects them.

## Creating the AI Agent Workflow

With our Slack MCP server hosted on Gram and Postiz MCP server configured, we need to build the AI agent that orchestrates the entire workflow. The agent will receive emoji reactions from Slack, analyze threads, and schedule social media posts automatically.

We'll create the agent logic in a separate module, then add a webhook endpoint to our FastAPI app that Slack's Events API will call when emoji reactions are added to messages.
...
Now we'll integrate our agent with FastAPI by adding a webhook endpoint that Slack's Events API will call when emoji reactions are added.
...
This endpoint handles two critical Slack events: URL verification (required when setting up the webhook) and emoji reactions. When someone adds a rocket emoji, it triggers our AI agent workflow in the background while immediately responding to Slack.
...
The webhook is now ready to receive Slack events and trigger our automated social media workflow.

### Configure the Slack Event URL

Before testing the integration, we need to configure Slack so it can reach our event endpoint and send emoji reaction notifications.
...
Save the changes, and we are now ready to test the integration. 

#### Step 3: Verify Bot Permissions

Ensure your bot has the required OAuth scopes under **OAuth & Permissions**:
...
With Slack configured to send reaction events to our webhook, we're ready to test the complete automated workflow.

## Testing the Integration 

Now let's test our complete automated workflow. Find a Slack thread with valuable content and add a 🚀 reaction to any message in that thread.
