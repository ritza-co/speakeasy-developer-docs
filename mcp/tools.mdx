...
<Callout title="Note" variant="success-gradient" fullWidth>
  The Python SDK also provides a higher-level interface called `FastMCP`, originally introduced in [FastMCP 1.0](https://github.com/jlowin/fastmcp/releases/tag/v1.0), which
  simplifies tool declarations. The example above uses the low-level API, which
  is recommended only when you need fine-grained control over the tool
  lifecycle.
</Callout>
