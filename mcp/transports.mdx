...
description: Learn how MCP Transports handle communication between clients and servers.
---
...
MCP currently supports three standard forms of transport: `stdio`, `SSE`, and `Streamable HTTP`. Custom transports that conform to the standard input and output (stdio) and message-handling semantics are also supported.
...
MCP servers can be exposed over three built-in transport mechanisms:
...
Streamable HTTP is a newer transport that supersedes SSE for production deployments. It provides better scalability, support for stateless operation, and improved resumability. It uses HTTP requests for both client-to-server and server-to-client communication.

- **Single endpoint** for all MCP communication
- **Multiple response modes** (batch JSON or streaming SSE)
- **Session management** with built-in tracking
- **Resumability** for broken SSE connections
- **Comprehensive authentication** support
- **Flexible CORS** configuration

Here is an implementation of an MCP server serving via Streamable HTTP:
...
- **DELETE**: For terminating sessions (when session termination is enabled)
- **OPTIONS**: For CORS preflight requests

#### Configuration Options

When using the HTTP Stream Transport, you can configure various options:

- **Port and endpoint**: The HTTP port to listen on and endpoint path
- **Response mode**: Choose between batch (JSON) or streaming (SSE) responses
- **Batch timeout**: Control how long to wait for responses in batch mode
- **Message size limit**: Set maximum allowed size for incoming messages
- **CORS configuration**: Comprehensive CORS support for web applications
- **Session management**: Built-in session tracking with configurable options
- **Stream resumability**: Support for resuming broken SSE connections

## Custom transport implementations

MCP does not require servers to use `stdio`, `SSE`, or `Streamable HTTP`. Any transport can be used, as long as it:
...
- Send progress updates with `notifications/progress`.
...
- For `SSE` and `Streamable HTTP`, **validate Origin headers**, enforce **authentication**, and **bind to `localhost`** during local development to avoid DNS rebinding vulnerabilities.
...
| Server UIs / browsers                     | `Streamable HTTP`          |
| Cross-network communication               | `Streamable HTTP` with Auth |
