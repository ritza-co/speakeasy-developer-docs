...
MCP resources give LLMs direct access to static, read-only data without requiring user approval for each request. Resource data can include documentation, configuration files, schemas, logs, or any reference material an LLM may need to consult during the conversation. Unlike MCP tools, which require user confirmation before execution, resources are treated as safe reference material that can be accessed freely.

For example, instead of manually copying a project's `README.md` file when asking an LLM installation questions, you can expose the README as a resource. The LLM can then access setup instructions directly whenever needed during the conversation, without interrupting the flow to ask for permission.
...
While both [MCP tools](/mcp/tools) and [MCP resources](/mcp/resources) can handle read-only operations, the key difference between them lies in their interaction models. Tools are treated as operations that may have side effects or require deliberate execution. Resources, by contrast, allow immediate access without user intervention. In most MCP clients (such as Claude Desktop, Copilot, or Cursor AI), tools require explicit approval or confirmation before execution, but resources can be accessed directly by the LLM without interrupting the conversation flow. 
...
- External API calls or processes that might incur costs.
- Actions where the user should be aware that something is happening.

Use MCP resources for:

- Configuration files, documentation, and schemas.
- Static reference data (catalogs, directories).
- Content the LLM might access multiple times during reasoning.
- Read-only data that should always be available to the LLM.

## Using the Sentry MCP Server with Claude Desktop

While [Claude Desktop](https://claude.ai/download) supports MCP resources on Pro and Team plans, it doesn't support resource templating, so you can't pass parameters to specify what content to retrieve dynamically.
