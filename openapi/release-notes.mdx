...
OpenAPI 3.2.0 (September 2025) is the newest feature release. It layers structured tag navigation, streaming-friendly media types, and fresh OAuth flows onto the JSON Schema-aligned foundation introduced in 3.1. Refer to the [official release notes](https://github.com/OAI/OpenAPI-Specification/releases/tag/3.2.0) for the canonical changelog.

## OpenAPI version history

| **Version**       | **Release date** | **Changes**                                                                                                                                                                                                                                                                                                                                                                                       | **Breaking changes** |
| ----------------- | ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------- |
| **OpenAPI 3.2.0** | Sep 19, 2025     | - Adds hierarchical tag metadata (`summary`, `parent`, `kind`) and reusable media types<br />- Supports streaming-friendly payloads with `itemSchema`, `query` HTTP operations, and `querystring` parameters<br />- Expands security (OAuth2 device flow, metadata URL), responses (`summary`), and server naming options | Limited              |
...
| --- | --- | --- |
| Tag navigation | Relies on `x-displayName` / `x-tagGroups` vendor extensions | Native `summary`, `parent`, and `kind` with registry-backed conventions |
...
| Documentation polish | Response `description` required, examples lack structured/serialized split | Optional response `description`, new `summary`, Example `dataValue`/`serializedValue`, server `name`, `$self` |

### What's new when you move to 3.2?

OpenAPI 3.2 keeps 3.1 descriptions valid while unlocking several headline upgrades. Here's what changes for teams planning the move:

- **Navigation metadata:** Standard tag fields (`summary`, `parent`, `kind`) replace the common `x-displayName`/`x-tagGroups` pattern. You can keep existing vendor extensions for backwards compatibility, but update doc toolchains so they favor the official fields.
- **Streaming support:** `itemSchema`, `prefixEncoding`, and sequential media type guidance make SSE, JSON Lines, and MCP channels first-class. Preview how your generators or SDKs render streamed payloads before switching production specs.
...
- **Docs & security polish:** Response `summary`, Example `dataValue`/`serializedValue`, reusable `components.mediaTypes`, server `name`, `$self`, OAuth device flows, metadata URLs, and URI-referenced schemes streamline governance. Adopt them incrementally; every feature is opt-in.

Ready for the deep dive? Head to "What's new in OpenAPI 3.2?" for worked examples and implementation notes.
...
If you're still on OpenAPI 3.0, moving straight to 3.2 lets you skip an intermediate upgrade while adopting both the 3.1 JSON Schema alignment and the 3.2 usability improvements. `3.1.0` was a mostly iterative release, with some breaking changes, whereas `3.2.0` is a feature release.
...
OpenAPI 3.2.0 folds years of community extensions directly into the specification. Use this deep dive when you're ready to implement the changes—the sections below walk through the navigation metadata, streaming semantics, and security tooling with snippets and migration tips.
...
### Server-Sent Events (SSE), MCP streams, and other streaming media types
...
- Model each SSE field (`event`, `id`, `retry`, `data`) in the schema so consumers know which keys to expect.
