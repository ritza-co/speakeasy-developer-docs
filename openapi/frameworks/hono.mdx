...
W'll explain how to add SDK creation to a CI/CD pipeline so that Speakeasy automatically generates fresh SDKs whenever your Hono API changes in the future.
...
If you want to follow along step-by-step, you can clone the [Speakeasy Hono example starter repository](https://github.com/speakeasy-api/hono-openapi-example.git), which has the initial state of the app that we'll use to start this tutorial.
</Callout>
...
[Overlays](/docs/prep-openapi/overlays/create-overlays) let you create overlay documents that can be merged with an OpenAPI document. This lets you update and use an OpenAPI document without modifying the original OpenAPI document, which is useful when the same OpenAPI document is used in multiple places. Common use cases include adding Speakeasy extensions, adding examples, and hiding internal APIs from a public SDK.
...
![Speakeasy Studio showing SDK improvement suggestions](./assets/hono/speakeasy-studio-suggestions.png)
