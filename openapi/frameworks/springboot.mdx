...
<Callout title="Example repository" type="info">
  ...
  To skip this setup and follow along with our example, clone the [example application](https://github.com/speakeasy-api/examples/tree/main/framework-springboot).
</Callout>
...
![Speakeasy Suggestions in Speakeasy Studio](./assets/springboot/speakeasy-suggestions.png)
...
The example app added retry logic to the SDK's `listPublications` operation to handle network errors gracefully. This was done using one of the OpenAPI extensions that [Speakeasy provides](/docs/speakeasy-reference/extensions), `x-speakeasy-retries`.
...
After re-creating the SDK using Speakeasy:
