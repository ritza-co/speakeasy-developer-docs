...
Actually, you can. We've built a [tool](/docs/model-context-protocol) that takes your OpenAPI document and turns it into an MCP server.
...
- An **OpenAPI document** is a YAML or JSON file that describes your API, from endpoints to fields to schemas for request payloads, successful responses, and errors. See more on our [OpenAPI Hub](/openapi).
...
Another solution to tool explosion is to disable the tools on the client side instead. Claude Desktop allows this, but if the server makes over 200 tools available, then you're probably still not going to have a great time toggling off each one that you don't want individually.
...
In the OpenAPI document snippet above, we have three endpoints that return information about users. The description of each of those operations is vague, which can lead to the LLM choosing the wrong endpoint when it needs to call one of them.

### Our solution to vague or long OpenAPI descriptions for MCP servers
...
To resolve that issue, Speakeasy enables the usage of `x-speakeasy-mcp`, a custom OpenAPI extension that you can use to describe endpoints to LLMs.
...
And if you don't want to pollute your OpenAPI document with extensions or non-native terminologies, you can use an [overlay](/openapi/overlays), which is essentially a separate document that modifies the OpenAPI document without directly editing the original document.
